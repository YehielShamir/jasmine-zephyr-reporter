...
export class FSystem {
  ...
  public async copyFilesToFolder(path: string, newFolderName: string) {
    // Valid folder name
    const newValidName = this.validName(newFolderName);
    ...
    if (filesName.length > 0) {
      for (const file of filesName) {
        // Create one
        if (!existsSync(resolve(path, newValidName)))
          await mkdir(resolve(path, newValidName), { recursive: true });
        await rename(resolve(path, file), resolve(path, newValidName, file));
      }
      return resolve(path, newValidName);
    }
    return -1;
  }
  ...
  /**
   * Ensure folder or file valid name
   */
  public validName(str: string) {
    return str.replace(/[*/\\:?"><|&=\n;]/g, '-');
  }
}
