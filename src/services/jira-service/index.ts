...
export default class JiraService extends HTTPClient {
   ...
   async getProjectId(projectIdOrKey: string | number): Promise<number> {
      ...
      try {
         const projectId = await this.methods.get(
            `/api/latest/project/${projectIdOrKey}`
         );
    ...
    */
   async getVersionIdByName(
      boardId: number,
      desiredVersionName: string
   ): Promise<number> {
      try {
         if (!boardId || !isNaN(boardId)) return -1;
         const allVersion = await this.methods.get(
            `/agile/latest/board/${boardId}/version`
         );
   ...
   async getIssueKey(issueIdOrKey: string | number, desiredKey: string): Promise<number> {
      ...
      try {
         ...
         return response.data && response.data[desiredKey]
            ? response.data[desiredKey]
            : -1;
      } catch (error) {
         return -1;
      }
